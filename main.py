...
def TitleWorkerr():
    ...
    ctypes.windll.kernel32.SetConsoleTitleW(f'xhulo | Generated : {genned} | Errors : {errors} | Solved : {solved} | Total : {round(genned + errors)}')
...
class Utils(object):
    ...
    def PostTokenInWebhook(token):
        for webhook in config["webhook_urls"]:
            try:
                httpx.post(webhook, json={ "username": "xhulo generator", "content": token})
...
class SolveCaptcha(object):
    def init(proxy, site_url, site_key, thread_id, can_solve_in_one_click, solver_address=None):
        ...
        captcha_key = httpx.post(solver_address, json={
            ...
            "proxy_url": 'ur-rotating-proxies-go-here'
        }, timeout=None).text
...
def GenerateToken(key, proxy, thread_id):
    try:
        ...
        account_password = "discordisshitlol1337"
        account_username = Utils.GetUsername() + ' | xhulo'
